...
const HospitalizationRiskPrediction = (props) => {
  ...
  const metrics = metricsData ? [
    ...
    { label: 'Hospitalization Risk Prediction Reliability', value: `${(metricsData.summary_metrics.alert_reliability * 100).toFixed(1)}%` },
    { label: 'Hospitalization Risk Detection Rate', value: `${(metricsData.summary_metrics.need_detection_rate * 100).toFixed(1)}%` },
    { label: 'Balanced Hospitalization Risk Prediction score', value: metricsData.summary_metrics.balanced_score.toFixed(2) }
  ] : [];
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    <Box display="grid" gridTemplateColumns="1fr 1fr" gap={2} mt={2}>
                      <Box bgcolor="#e0f2f1" p={2} borderRadius={2} textAlign="center">
                        ...
                        <Typography variant="body2">Correctly predicted Hospitalization risk</Typography>
                      </Box>
                      <Box bgcolor="#ffebee" p={2} borderRadius={2} textAlign="center">
                        ...
                        <Typography variant="body2">Missed Hospitalization predictions</Typography>
                      </Box>
                      <Box bgcolor="#ffebee" p={2} borderRadius={2} textAlign="center">
                        <Typography variant="h6">{metricsData.confusion_matrix.false_positive}</Typography>
                        <Typography variant="body2">Unnecessaty Hospitalization Predictions</Typography>
                      </Box>
                      <Box bgcolor="#e0f2f1" p={2} borderRadius={2} textAlign="center">
                        <Typography variant="h6">{metricsData.confusion_matrix.true_negative}</Typography>
                        <Typography variant="body2">Accurately Identified as Not Needing Hospitalization</Typography>
                      </Box>
                    </Box>
                  </Grid>
...
export default HospitalizationRiskPrediction;
